...
export class formGridComponent implements OnChanges, OnInit, AfterViewInit {
  ...
  loadGrid(src?: string) {
    ...
    // Do not double load.
    if (this.form && this.src && (src === this.src)) {
      return;
    }

    if (src) {
      this.src = src;
      this.form = new formPromiseService(this.src, { formOnly: true });
    }

    // Load the header.
    this.header.load(this.form, {}, this.columns)
      .then(() => this.setPage(0))
  ...
  refreshGrid(query?: any) {
    ...
    if (this.items) {
      ...
      loader = this.body.load(this.form, this.query);
    }
